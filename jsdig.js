...
if (['/help', 'help', '-help', '-h', 'h', '?', '-?', '/?'].indexOf(args[2]) > -1) {
  ...
  Object.prototype.joinAnswers = function (contextClue) {
    ...
    for (let i = 0; i <= answerKeys.length - 1; i++) {
      ...
      switch (i) {
        case 0: {
          coloredText = '\033[32;1;4m' + answerValues[i];
          break;
        }; // Name
      case 1: {
        coloredText = '\033[34;1m' + answerValues[i];
        break;
      }; // TTL
      case 2: {
        coloredText = '\033[38;5;220m' + classTranslate(answerValues[i], 'TYPE');
        break;
      }; // Type
      case 3: {
        coloredText = '\033[36;1m' + classTranslate(answerValues[i], 'CLASS');
        break;
      }; // Class
      default: {
        coloredText = '\033[33m' + answerValues[i];
        break;
      }; // All data fields
      }
